...
import {
    ...
    updateEscrowStatus,
    submitEscrow, completeEscrow, reopenEscrow,
...
router.get("/me",           validateToken(["student", "local"]), getMyEscrows);  // frontend alias
router.get("/:id",          validateToken(),                     getEscrowById);
router.put("/:id/status",   validateToken(),                     updateEscrowStatus);
router.put("/:id/submit",   validateToken(["student"]),          submitEscrow);
